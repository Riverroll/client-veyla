...
const CartPopup = ({ setIsPopupOpen }) => {
  const [isOpen, setIsOpen] = useState(false); // Status untuk menampilkan popup
  const [isCheckoutModalOpen, setIsCheckoutModalOpen] = useState(false); // Status untuk menampilkan modal checkout
  const cartItems = useSelector((state) => state.cart?.items ?? []); // Mengambil item dalam keranjang dari Redux
  const dispatch = useDispatch(); // Mendapatkan fungsi dispatch dari Redux

  // Mengatur status popup berdasarkan status popup dan modal checkout
  useEffect(() => {
  ...
  // Fungsi untuk memastikan nilai yang dikonversi adalah float yang aman
  ...
  const calculateTotal = () => {
    return cartItems.reduce(
      (total, item) =>
        total + safeParseFloat(item.product_price) * item.quantity,
      0
    );
  };
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        Quantity: {item.quantity} x IDR
                        {safeParseFloat(item.product_price).toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <button
